...
**Methods:** We analyzed 1,884,793 police-recorded traffic accidents across Japan (2019-2024), comparing accident counts on 10 Friday the 13ths against other Fridays. We reproduced three established Western analytical approaches (Scanlon paired comparison, Näyhä negative binomial regression, and Lo multi-date ANOVA) and added three original analyses: (i) a case-crossover analysis on same-month Friday strata using three complementary tests (a month-stratified conditional Poisson regression, an exact sign test, and a sign-flip permutation test); (ii) a covariate-adjusted 47-prefecture negative binomial panel model with weather and holiday adjustment; and (iii) 47 prefecture-specific negative binomial fits summarized as a forest plot (Figure S3). Subgroup analyses examined accident severity, age groups, and time of day. Full statistical detail—including cluster-robust standard errors, small-cluster corrections, and residual diagnostics—is in Methods and the Supplementary Materials.

**Results:** Across all analytical approaches, no large Friday the 13th effect was detected. The Scanlon-method rate ratio was 1.04 (paired t-test p = 0.47), the Näyhä-method count ratio was 1.01 (95% confidence interval [CI], 0.54-1.91; p = 0.97), and the Lo-method ANOVA showed no between-group differences (F = 0.16, p = 0.93). The month-stratified conditional Poisson case-crossover regression gave a count ratio of 1.04 (95% CI, 0.95-1.14, sandwich-t(G-1) p = 0.40); the companion exact sign test (8 of 10 pairs favored Fri13, p = 0.11) and sign-flip permutation test (exact p = 0.45 across all 1024 possible sign patterns; geometric count ratio 1.04) were also null. The 47-prefecture negative binomial panel model with weather and holiday adjustment yielded a count ratio of 1.02 (95% CI, 0.97-1.08, two-way cluster p = 0.36); the confidence interval was centered near the null and excluded effects of the magnitude reported in early Western studies, although very small effects could not be ruled out. Prefecture-specific fits ranged from 0.59 (Tottori) to 1.29 (Iwate) with a median of 1.01; 5 of 47 nominally exceeded the null (binomial excess-test p = 0.08 versus α=0.05), and after Bonferroni or Benjamini-Hochberg correction only Mie prefecture remained significant. Fatal accidents restricted to Friday-vs-Friday comparisons showed a crude rate ratio of 1.31 (Welch t-test raw p = 0.09, Bonferroni-adjusted p = 0.70), with a Poisson-approximation NB-adjusted count ratio of 1.23 (95% CI, 1.000-1.506, marginal lower bound) that did not survive multiple-comparison correction across the 8 pre-specified subgroups. All sensitivity analyses excluding COVID-period data, holidays, and Obon/New Year periods yielded consistent null results (RR range: 1.01-1.06).
...
*"Same-month other Fridays" refers to the arithmetic mean of all Fridays in the same calendar month excluding the Friday the 13th itself (typically Fri 6, 20, and 27). This within-month control set differs from the "all other Fridays" comparison used in the Results text and Table 2, which pools all 303 non-13th Fridays across the study period, and from the Scanlon-method comparison (Friday 13th vs. same-month Friday 6th only). The bolded "Mean (arithmetic per-pair ratio)" row is the arithmetic average of the ten per-pair case-to-control ratios (1.05) — an alternative summary reported for reader transparency but not used as an inferential estimand. The "Geometric-mean count ratio" row (1.04) is exp(mean of the ten log-ratios) and is the estimand of the sign-flip permutation and conditional-Poisson case-crossover tests reported in the Methods and Results. The two summaries are numerically close because the per-pair ratios sit near 1, but they answer slightly different statistical questions and are not interchangeable in downstream inference.*
...
|----------|-----------|----------------|----------|---------------|-------------------|----------------------------------|------|
| Fatal accidents | 9.90 | 7.57 | 1.307 | 0.087 | 0.696 | 1.227 [0.9997, 1.5061] † | 0.050 |
...
**Figure S1.** Pearson residual distribution for the primary weather-and-holiday-adjusted NB2 panel model (n = 103,008 prefecture-days). Empirical mean = +0.000, standard deviation = 1.01; 4.4% of residuals exceed |r| > 2 (versus 4.55% expected under a standard normal) and 0.69% exceed |r| > 3 (versus 0.27% expected). The mild fat tail beyond ±3 is consistent with a small number of extreme-event days that count regression cannot fully absorb and does not indicate systematic model misspecification for the Friday-the-13th estimand.

**Figure S2.** Same-month pair plot for the 10 Fri13-containing months of 2019-2024. Each connecting line represents one month (10 lines total); the left endpoint (red diamond) is the daily accident count on Friday the 13th and the right endpoint (blue circle) is the mean of the other Fridays in the same calendar month. Eight of ten pairs favor Fri13 > control (consistent with the exact sign test result reported in the Results, 8 of 10 pairs, p = 0.11), including a visible downward outlier in August 2021 (Obon period).
